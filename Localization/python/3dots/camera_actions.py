...
blue_lower = np.array([75,100,100])
blue_upper = np.array([130,255,255])

yellow_lower = np.array([20,100,100])
yellow_upper = np.array([40,255,255])

violet_lower = np.array([130,100,100])
violet_upper = np.array([160,255,255])

orange_lower = np.array([0, 100, 100])
orange_upper = np.array([20,255,255])

def ID_hue_image(img, ID):
	if(ID is 1):
		lower_red_hue_range = cv2.inRange(img, cv2.cv.Scalar(0, 100, 100), cv2.cv.Scalar(10, 255, 255))
		upper_red_hue_range = cv2.inRange(img, cv2.cv.Scalar(160, 100, 100), cv2.cv.Scalar(180, 255, 255))
		hue_image = cv2.addWeighted(lower_red_hue_range, 1.0, upper_red_hue_range, 1.0, 0.0)
		return hue_image
	elif(ID is 2):
		lower = green_lower
		upper = green_upper
	...
	hue_image = cv2.inRange(img, lower, upper)
	hue_image = cv2.GaussianBlur(hue_image, (9,9), 2, 2)
...
def main():
	if len(sys.argv) == 2:
		...
		hue_image = ID_hue_image(hsv_image, Robot1.ID)
