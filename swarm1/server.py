...
# Create four robots
robot1 = Robot(BLUE, BLUE, BLUE) #ROBOT 0
robot2 = Robot(YELLOW, YELLOW, YELLOW) #ROBOT1
robot3 = Robot(RED, RED, RED)        # ROBOT2
...
robot1.HOST = '192.168.1.9'  # blue robot
robot2.HOST = '192.168.1.4'  # red red blue
robot3.HOST = '192.168.1.9'  # red robot
robot4.HOST = '192.168.1.9'  # Yellow Robot
...
while True:
    ...
    if i > 10:
        ...
        if ROBOT_MODE == RENDEZVOUS:
            (XPOS[0], YPOS[0], ANGLE[0]) = rendezvous(ROBOTS[0], ROBOTS[0], ROBOTS[0])
            (XPOS[1], YPOS[1], ANGLE[1]) = rendezvous(ROBOTS[1], ROBOTS[0], ROBOTS[0])
            (XPOS[2], YPOS[2], ANGLE[2]) = rendezvous(ROBOTS[2], ROBOTS[0], ROBOTS[0])
            (XPOS[3], YPOS[3], ANGLE[3]) = rendezvous(ROBOTS[3], ROBOTS[0], ROBOTS[0])
        ...
        for k in range(0, len(ROBOTS)):
            print(
            k, "MESSAGE:", MESSAGES[k], "Robot xpos:", ROBOTS[k].xpos, "Robot ypos:", ROBOTS[k].ypos, "Robot dir:",
            ROBOTS[k].dir,
